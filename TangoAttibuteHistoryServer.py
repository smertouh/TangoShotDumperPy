...
class TangoAttributeHistoryServer(TangoServerPrototype):
    ...
    def configure_attribute(self, name, param=None):
        ...
        conf = {'ready': False, 'attribute': None, 'local_name': name.replace('/', '_'), 'device_proxy': None}
        ...
        # check if remote attribute polled
            ...
            try:
                ...
                if conf['ready']:
                    if conf['attribute'] is None:
                        ...
                        conf['attribute'] = attr
                        # set local attr info according to rwmote arrt
    ...
    def read_attribute(self, attr: tango.Attribute):
        name = attr.get_name()
        conf = self.config.attributes[name]
        if not conf['ready']:
            # reconnect to attribute
           self.configure_attribute(name)
        if not conf['ready']:
